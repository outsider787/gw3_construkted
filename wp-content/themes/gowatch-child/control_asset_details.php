	...
	if( isset($_REQUEST['post_id']) && !empty($_REQUEST['post_id']) ) {
	    ...
	    if($thumbnail_id != "")
	        if( ! wp_delete_attachment( $thumbnail_id, true )) {
	            echo json_encode(array('errCode' => 1, 'errMsg' => 'failed to delete old thumbnail!'));
	            exit;
	        }
        ...
        if(isset($_REQUEST['count_of_triangles']))
            $mesh_polygon_count = $_REQUEST['count_of_triangles'];

        $ret = update_post_meta( $post_id, 'mesh_polygon_count', $mesh_polygon_count);

        if($ret == false){
            echo json_encode(array('errCode' => 1, 'errMsg' => 'failed to update mesh polygon count meta field!'));
            exit;
        }
